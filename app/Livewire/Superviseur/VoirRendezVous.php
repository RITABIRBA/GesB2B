...
class VoirRendezVous extends Component
{
    ...
    // ─── Pauses (pour trouverCreneauDisponible) ───────────
    ...
    private function calculerCompatibiliteProfils(Participant $a, Participant $b): int
    {
        ...
        if ($matchSecteur) $points++;
        elseif (empty($secteursA) && empty($secteursB)) $points++;
    ...
    /**
     * ✅ CORRECTION BUG — cherche un créneau disponible
     * (identique à l'Admin) au lieu de créer un RDV sans date.
     */
    ...
    /**
     * ✅ CORRECTION BUG — utilise trouverCreneauDisponible()
     * et crée le RDV avec date/heure/salle/table + statut 'planifie'
     */
    public function creerMatchManuel(): void
    {
        ...
        // ✅ CORRECTION : cherche un créneau disponible
        ...
        if (!$creneau) {
            $this->addError('match_participant2', 'Aucun créneau disponible pour ces deux participants.');
            return;
        }

        // ✅ CORRECTION : crée le RDV avec date/heure/salle/table
        RendezVous::create([
            ...
            'statut'          => 'planifie', // ✅ CORRECTION : 'planifie' au lieu de 'a_planifier'
        ]);
        ...
        Notification::create([
            ...
            'contenu'        => "📅 Un rendez-vous a été organisé par la supervision avec " . ($p2->nom ?? '') . ' ' . ($p2->prenom ?? '') . " {$infoCreneau}.",
        ...
        Notification::create([
            ...
            'contenu'        => "📅 Un rendez-vous a été organisé par la supervision avec " . ($p1->nom ?? '') . ' ' . ($p1->prenom ?? '') . " {$infoCreneau}.",
            'date_envoie'    => now()->toDateString(),
            'type'           => 'systeme',
        ]);
    ...
    public function confirmerAnnulation(): void
    {
        ...
        if ($absent) {
            Notification::create([
                ...
                'contenu'        => "Votre rendez-vous avec " . ($autre->nom ?? '') . ' ' . ($autre->prenom ?? '') . " a été annulé par la supervision (absence).",
                'date_envoie'    => now()->toDateString(),
                'type'           => 'systeme',
            ]);
        }
        ...
        if ($autre) {
            Notification::create([
                ...
                'contenu'        => "⚠️ Votre rendez-vous avec " . ($absent->nom ?? '') . ' ' . ($absent->prenom ?? '') . " a été annulé (absence signalée).",
                'date_envoie'    => now()->toDateString(),
                'type'           => 'systeme',
            ]);
        }
    ...
    public function effectuerRematch(): void
    {
        ...
        if ($conflit) {
            ...
            $this->erreur_rematch = ($participant->nom ?? '') . ' ' .
                ($participant->prenom ?? '') .
                ' a déjà un RDV sur ce créneau. Choisissez un autre participant !';
            return;
        }
